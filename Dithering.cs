...
namespace DrawApp
{
    class Dithering
    {
        ...
        private static byte[,,] Dither(byte[,,] ditherImage, int factor)
        {
            ...
            {
                for (int x = 1; x < ditherImage.GetLength(1) - 2; x++)
                {
                    ...
                    SetPx(ditherImage, x + 1, y, 1, err);
                    SetPx(ditherImage, x + 2, y, 1, err);
                    SetPx(ditherImage, x - 1, y + 1, 1, err);
                    SetPx(ditherImage, x, y + 1, 1, err);
                    SetPx(ditherImage, x + 1, y + 1, 1, err);
                    SetPx(ditherImage, x, y + 2, 1, err);

                    if (y == ditherImage.GetLength(0) - 3)
                    {
                        ditherImage[y + 1, x, 0] = ditherImage[y - 1, x, 0];
                        ditherImage[y + 1, x, 1] = ditherImage[y - 1, x, 1];
                        ditherImage[y + 1, x, 2] = ditherImage[y - 1, x, 2];
                        ditherImage[y + 2, x, 0] = ditherImage[y, x, 0];
                        ditherImage[y + 2, x, 1] = ditherImage[y, x, 1];
                        ditherImage[y + 2, x, 2] = ditherImage[y, x, 2];
                    }
                    if (x == ditherImage.GetLength(1) - 3)
                    {
                        ditherImage[y, x + 1, 0] = ditherImage[y, x - 1, 0];
                        ditherImage[y, x + 1, 1] = ditherImage[y, x - 1, 1];
                        ditherImage[y, x + 1, 2] = ditherImage[y, x - 1, 2];
                        ditherImage[y, x + 2, 0] = ditherImage[y, x, 0];
                        ditherImage[y, x + 2, 1] = ditherImage[y, x, 1];
                        ditherImage[y, x + 2, 2] = ditherImage[y, x, 2];
                    }
